...
class CustomerRank(models.Model):
    ...
    def name_get(self):
        ...
        return result
    min_hino_vehicles = fields.Integer(
        string='Minimum Hino Vehicles', required=True)
    max_hino_vehicles = fields.Integer(
        string='Maximum Hino Vehicles', required=True)
    min_owned_vehicles = fields.Integer(
        string='Minimum Owned Vehicles', required=True)
    max_owned_vehicles = fields.Integer(
        string='Maximum Owned Vehicles', required=True)
        ...
        for record in self:
            ...
            if record.min_hino_vehicles > record.max_hino_vehicles:
                raise ValidationError(
                    "Minimum Hino Vehicles cannot be greater than Maximum Hino Vehicles.")

            if record.min_owned_vehicles > record.max_owned_vehicles:
                raise ValidationError(
                    "Minimum Owned Vehicles cannot be greater than Maximum Owned Vehicles.")

            if record.max_hino_vehicles > record.max_owned_vehicles:
                raise ValidationError(
                    "Maximum Hino Vehicles cannot be greater than Maximum Owned Vehicles.")

            if record.min_hino_vehicles > record.min_owned_vehicles:
                raise ValidationError(
                    "Minimum Hino Vehicles cannot be greater than Minimum Owned Vehicles.")
