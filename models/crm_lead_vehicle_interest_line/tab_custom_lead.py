...
class VehicleInterest(models.Model):
    ...
    # x_model_id = fields.Many2one('product.product', string='Vehicle Type', required=True )
    x_model_id = fields.Many2one('product.product', string='Vehicle Type', required=True,
                                 # domain="[('id', '=', sale_detail_ids.x_product_id.id)]"
                                 )
    ...
    @api.onchange('sale_detail_ids')
    def _onchange_sale_detail(self):
        if self.sale_detail_ids:
            self.x_model_id = self.sale_detail_ids.x_product_id

    # @api.onchange('x_partner_code')
    # def _onchange_partner_code(self):
    #     if self.x_partner_code:
    #         self.x_partner_name = self.x_partner_code.name
    #         self.x_address = self.x_partner_code.contact_address_complete or ''
    #         self.x_province_id = self.x_partner_code.state_id

    @api.onchange('sale_request_id')
    def _onchange_sale_request(self):
        if self.sale_request_id:
            self.x_expected_implementation_time = self.sale_request_id.x_expected_sale_date
            self.x_expected_time_sign_contract = self.sale_request_id.x_expected_to_sign_contract